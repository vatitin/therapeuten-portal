...
describe('TherapistController', () => {
  ...
  const dummyTherapist = { id: '123', therapistName: 'testTherapist', sub: 'testUUID' };
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      ...
      providers: [{ provide: TherapistService, useValue: mockTherapistService }],
    })
  ...
  describe('getProfile', () => {
    it('should return profile information for an authenticated therapist', async () => {
      // Arrange
      const expectedProfile = { id: '123', therapistName: 'testUser', info: 'Profile Data' };
      ...
      expect(mockTherapistService.getProfile).toHaveBeenCalledWith(dummyTherapist);
  ...
  describe('createPatient', () => {
    it('should create a new patient and return it', async () => {
      ...
      mockTherapistService.createPatient.mockResolvedValue(createdPatient);

      // Act
      const result = await controller.createPatient(status, therapist, dummyPatientDTO);

      // Assert
      expect(mockTherapistService.createPatient).toHaveBeenCalledTimes(1);
      expect(mockTherapistService.createPatient).toHaveBeenCalledWith(dummyPatientDTO, therapist.sub, status);
  ...
  describe('updatePatient', () => {
    ...
    it('should update patient and return updated info', async () => {
      ...
      mockTherapistService.updatePatient.mockResolvedValue(updatedPatient);

      // Act
      const result = await controller.updatePatient(patientId, status, patientDTO, therapist);

      // Assert
      expect(mockTherapistService.updatePatient).toHaveBeenCalledTimes(1);
      expect(mockTherapistService.updatePatient).toHaveBeenCalledWith(patientId, patientDTO, therapist.sub, status);
    ...
    it('should propagate error if service throws (e.g. patient not found)', async () => {
      ...
      mockTherapistService.updatePatient.mockRejectedValue(new Error(errorMessage));

      // Act & Assert
      await expect(
        controller.updatePatient('non-existent-id', status, patientDTO, therapist)
  ...
  describe('removePatient', () => {
    it('should call service to remove a patient', async () => {
      ...
      mockTherapistService.removePatient.mockResolvedValue({ success: true });
      ...
      expect(mockTherapistService.removePatient).toHaveBeenCalledWith(patientId, user.sub);
  ...
  describe('getPatientsFromTherapist', () => {
    it('should return a list of patients with the given status', async () => {
      ...
      mockTherapistService.getPatientsFromTherapist.mockResolvedValue(patients);

      // Act
      const result = await controller.getPatientsFromTherapist(status, therapist);

      // Assert
      expect(mockTherapistService.getPatientsFromTherapist).toHaveBeenCalledTimes(1);
      expect(mockTherapistService.getPatientsFromTherapist).toHaveBeenCalledWith(therapist.sub, status);
  ...
  describe('getPatient', () => {
    it('should return a single patient by id', async () => {
      ...
      mockTherapistService.getPatient.mockResolvedValue({ patient: patientData });
      ...
      expect(mockTherapistService.getPatient).toHaveBeenCalledWith(patientId, therapist.sub);
